"""SSE streaming helpers for the Pick AI agent.

Provides stream_agent_response() which wraps agent.stream() v2 output
into PRD-compliant SSE events: text, shop_card, done, error.
"""
...
logger = logging.getLogger("pick.agent.chat")
...
async def stream_agent_response(
    ...
    *,
    ...
    Returns:
        The session_id from config for use in the done event.
    """
    ...
    try:
        async for chunk in agent.astream(
            ...
            if chunk_type == "messages":
                # Token-level LLM output: (message_chunk, metadata) tuple
                ...
                # Structured events from tools (shop_card, status, etc.)
